         ...
         if(document.getElementById("toastFile").value != "") {
             if(document.getElementById("toastFile").files[0].type == "text/csv"){
                 ...
                 reader.onload = function(e) {
                     ...
                     dat.forEach(function(item, index, array) {
                         curr= array[index];
                         content = content + '<div class="box box-primary"><div class="box-header"><h3 class="box-title">' + curr.Employee + '</h3></div><div class="box-body"><ul><li>Hours Worked: ' + curr["Regular Hours"] + '</li><li>Tips Earned: $' + Math.floor(tips / parseFloat(curr["Regular Hours"])) + '</li></div></div>';
                         left = left + ((tips / parseFloat(curr["Regular Hours"])) - Math.floor(tips / parseFloat(curr["Regular Hours"])));
                     });
                     content = content + '<p>Remaining: $' + Math.floor(left) + '</div></div></section>';
